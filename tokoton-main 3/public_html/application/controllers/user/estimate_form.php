...
#				$this->output->enable_profiler(TRUE);
			...
			if(isset($_SESSION['form']['car_arrival_date']) and $_SESSION['form']['car_arrival_date'] != ""){
				...
				$data['check_pulldown2'] = $this->estimate_formmodel->car_check_pulldown2($selected_y2,$selected_m2,$selected_d2);
			}else{
				$data['check_pulldown2'] = $this->estimate_formmodel->car_check_pulldown2();
			}
			
			// 時間帯プルダウン作成
			if(isset($_SESSION['form']['time_zone']) and $_SESSION['form']['time_zone'] != ""){
				$data['time_zone_pulldown'] = $this->estimate_formmodel->time_zone_pulldown($_SESSION['form']['time_zone']);
			}else{
				$data['time_zone_pulldown'] = $this->estimate_formmodel->time_zone_pulldown();
			}
			/* 20161219@nagai END */
			...
			/*SEO対策 [PUBLIC_TOKOTON-3] /ありページへcanonical対応 20170420 ST*/
			if(isset($_SERVER['HTTPS']) and $_SERVER['HTTPS'] == "on"){
				$data['canonical'] = "<link rel=\"canonical\" href=\"https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."\" />";
			} else {
				$data['canonical'] = "<link rel=\"canonical\" href=\"http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."\" />";
			}
			/*SEO対策 [PUBLIC_TOKOTON-3] /なしページへcanonical対応 20170420 ED*/
			...
			if(!empty($not_open) and $not_open == "t"){
				...
				if(user_agent()){
					$this->smarty_parser->parse("ci:user/estimate_form_sp.tpl", $data);
				}else{
...
#			$data['document_root'] = $_SERVER['DOCUMENT_ROOT'];
			...
			$data['mainmenu_list'] = "mainmenu_list_act";
			$data['nowyear'] = date("Y");
			$data['page_name'] = "estimate_confirm";
			...
			if(isset($_SESSION['form']) and $_SESSION['form'] != null){
				...
				*/
				...
				if($check_y2 != "" and $check_m2 != "" and $check_d2){
					...
					//不明の場合は01に置き換える
					$wk_date = date('Y/m/d H:i:s',strtotime(str_replace('不明', '01', $_SESSION['form']['car_arrival_date'])));
					
					if($wk_date >= '2019/05/01 00:00:00'){
						
						$arrival_date = "令和".($check_y2 - 2018)."年".$check_m2."月";
						$arrival_date = str_replace('令和1', '令和元', $arrival_date);
						
					}else{
						$arrival_date = "平成".($check_y2 - 1988)."年".$check_m2."月";
					}
					
					if($check_d2 != "不明"){
						$arrival_date .= $check_d2."日";
					}
				}
				
				if(isset($arrival_date)){
					$data['form_data']['arrival_date'] = $arrival_date;
				}
				//令和元号対応 ED
				...
				*/
				...
				//var_dump($_SESSION['form']['car_check_date']);
				
				if($check_y != "" and $check_m != "" and $check_d){
					
					//不明の場合は01に置き換える
					$wk_date2 = date('Y/m/d H:i:s',strtotime(str_replace('不明', '01', $_SESSION['form']['car_check_date'])));
					
					if($wk_date2 >= '2019/05/01 00:00:00'){
						
						if($check_y >= 2019 and $check_y <= 2022){
							$check_date = "令和".($check_y - 2018)."(平成".($check_y - 1988).")年".$check_m."月";
						}else{
							$check_date = "令和".($check_y - 2018)."年".$check_m."月";
						}
						
						//$check_date = "令和".($check_y - 2018)." (平成".($check_y - 1988).") 年".$check_m."月";
						$check_date = str_replace('令和1', '令和元', $check_date);
						
					}else{
						$check_date = "平成".($check_y - 1988)."年".$check_m."月";
					}
					
					if($check_d != "不明"){
						$check_date .= $check_d."日";
					}
				}
				
				$data['form_data']['check_date'] = $check_date;
				//令和元号対応 ED
...
#					$select_culum = "db_menu_nm,db_menu_detail,dd_dsc_nm";
			...
			if(user_agent()){
				$this->smarty_parser->parse("ci:user/estimate_confirm_sp.tpl", $data);
			}else{
		...
		function sendmail(){
			...
			// 2019/02/07 テスト用 ST
			// メール代理応答サービス用センターメールアドレス設定
#			$represent_mail = "xanone4@gmail.com";									// テスト
#			$log_mail = "enonax@yahoo.co.jp";
...
# 			$add1_mail = "les-papillons-coeurs@hotmail.co.jp";
			...
			if(isset($_SESSION['form']) and $_SESSION['form'] != null){
				...
				if(isset($_SESSION['form']['car_arrival_date']) and $_SESSION['form']['car_arrival_date'] != ""){
					...
					if($check_y2 != "" and $check_m2 != "" and $check_d2 != ""){
						...
						if($wk_date >= '2019/05/01 00:00:00'){
							...
							$check_date2 = "令和".($check_y2 - 2018)."年".$check_m2."月";
							$check_date2 = str_replace('令和1', '令和元', $check_date2);
						
						}else{
							$check_date2 = "平成".($check_y2 - 1988)."年".$check_m2."月";
						}
						
						if($check_d2 != "不明"){
							$check_date2 .= $check_d2."日";
						}
					}
					//現状処理コメントアウト
					/*
				...
				if(isset($_SESSION['form']['car_check_date']) and $_SESSION['form']['car_check_date'] != ""){
					...
					if($check_y != "" and $check_m != "" and $check_d != ""){
						//不明の場合は01に置き換える
						$wk_date = date('Y/m/d H:i:s',strtotime(str_replace('不明', '01', $_SESSION['form']['car_check_date'])));
...
// 2011/07/13 globalcoms					$mail_to = $mail.",".$log_mail.",".$add1_mail;
				...
				/*20161227@nagai ST*/
				$mail_body .= "※実際の入庫日は、店舗とご相談の上決定となります。\n";
				/*20161227@nagai END*/
				$mail_body .= "[その他、ご要望]\n";
...
#				$send_result = mail('jinji@globalcoms.co.jp',$mail_subject,$discription.$mail_body,"From:".$mail_from_tks."\nMime-Version: 1.0\nContent-Type: text/plain; charset=ISO-2022-JP\nContent-Transfer-Encoding: 7bit");
...
//20100407 SEO会社変更のためコンバージョンタグ変更 end
			...
			if(user_agent()){
				$this->smarty_parser->parse("ci:user/estimate_fin_sp.tpl", $data);
			}else{
