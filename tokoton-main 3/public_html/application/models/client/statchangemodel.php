<?
	class Statchangemodel extends Model {
		var $table = "t_shop_base";
		var $rows;
		
		function Statchangemodel(){
			parent::Model();
...
#			$this->db->orderby("mail_lastupdate DESC");
		...
		function do_update($list = array()){
			...
			if($list['sid'] != ""){
				$sid = $list['sid'];
				if(isset($list['sd_exam_state'])){
					$atrlist['sd_exam_state'] = $list['sd_exam_state'];
				}elseif(isset($list['sd_open_state'])){
					$atrlist['sd_open_state'] = $list['sd_open_state'];
				}
