{include file="ci:admin/header.tpl"}
  ...
  <div id="content">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" id="detail">
        ...
        <tr>
          <td width="246"><ul>
              <li><a href="http://www.tokoton-navi.com/shop_detail/{$sid}/manager/" target="_blank" class="btn_open">店舗ページを見る &gt;&gt; </a></li>
		...
		{html_select_date display_ymd=true prefix="sd_start_m" time=$query_data.sd_start_m start_year="-3" end_year=+3 display_days=false month_format=%m field_order=YMD}
	      ...
	      <tr>
	      	<td class="subdetail_label">料金テーブル</td>
	      	<td class="subdetail_value">
			<form action="/admin/detail/to_confirm" method="post">
			<select name="sd_copy_lank">
			{$rank_pulldown}
			</select>
			<input type="submit" name="submit" value="変更" />
			<input type="hidden" name="sid" value="{$sid}" />
			<input type="hidden" name="mode" value="copy_rank" />
			</form>
		</td>
	      </tr>
            </table>
		  ...
		  {/if}
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="subdetail">
              ...
              <tr>
                ...
                <td width="126" class="subdetail_value"><input type="submit" name="Submit3" value="解除" onclick="if(formConfirm('update')) location.href='/admin/detail/change_status/sd_exam_state-0/{$sid}'" /></td>
              </tr>
              <tr>
		...
		{/if}
          ...
          <td>
			...
			<table border="0" cellspacing="0" cellpadding="0" class="subdetail_right">
              ...
              <tr>
                <td class="subdetail_label">パスワード</td>
                <td class="subdetail_value">{$query_data.sd_pwd}</td>
              </tr>
            </table>
	    ...
	    <input type="hidden" name="mode" value="memo_regist" />
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="subdetail_right">
              ...
              <tr>
                <td class="subdetail_label">サービスアイコン</td>
                <td class="subdetail_value">
				{if $icon.icon1 == "t"}
<!--				<img src="/img/admin/icon_1.gif" alt="とことん24取扱い" width="32" height="32" />-->
			  ...
			  <tr>
			  	<td class="subdetail_label">対応クレジットカード</td>
			  	<td class="subdetail_value">
				{if $query_data.card1 == "t"}
					<img src="/img/admin/visa.gif" alt="VISA" />
				{/if}
				{if $query_data.card2 == "t"}
					<img src="/img/admin/master.gif" alt="MasterCard"  />
				{/if}
				{if $query_data.card3 == "t"}
					<img src="/img/admin/jcb.gif" alt="JCB" />
				{/if}
				{if $query_data.card4 == "t"}
					<img src="/img/admin/dc.gif" alt="DC" />
				{/if}
				{if $query_data.card5 == "t"}
					<img src="/img/admin/ae.gif" alt="AMERICANEXPRESS" />
				{/if}
				{if $query_data.card6 == "t"}
					<img src="/img/admin/uc.gif" alt="uc" />
				{/if}
				{if $query_data.card7 == "t"}
					<img src="/img/admin/nicos.gif" alt="Nicos" />
				{/if}
				{if $query_data.card8 == "t"}
					<img src="/img/admin/diners.gif" alt="Diners" />
				{/if}
				{if $query_data.card9 == "t"}
					<img src="/img/admin/other.gif" alt="その他" width="40" height="38" />
				{/if}
				&nbsp;
				</td>
			  </tr>
		...
		<select name="sd_shop_rank">
		  ...
		  <input type="hidden" name="mode" value="rank_regist" />
			  ...
			  <tr>
                ...
                <td class="subdetail_value"><a href="{$query_data.sd_shop_url}" target="_blank">{$query_data.sd_shop_url}</a>{if $query_data.sd_shop_url == ""}&nbsp;{/if}</td>
              </tr>
			  <tr>
			  ...
			  <tr>
                <td class="subdetail_label">地図座標</td>
                <td class="subdetail_value">
					<form action="/admin/detail/pointRegist/" method="post">
					緯度：<input type="text" name="sd_point_la" value="{$query_data.sd_point_la}" />&nbsp;&nbsp;経度：<input type="text" name="sd_point_lon" value="{$query_data.sd_point_lon}" />&nbsp;<input type="submit" name="submit" value="座標適用" />
						<input type="hidden" name="sid" value="{$sid}" />
						<input type="hidden" name="mode" value="point_regist" />
					</form>
				</td>
              </tr>
			  <tr>
                <td class="subdetail_label" rowspan="2">ページタイトル</td>
                <td class="subdetail_label"><span class="red">※35文字を目処に、地名＋車検＋店舗名を含め設定してください。</span><br />(例)　横浜市の車検ならスーパーFLEX車検へお任せください｜とことん車検ナビ</td>
			  </tr>
			  <tr>
                <td class="subdetail_value">
					<form action="/admin/detail/locate_title_change/" method="post">
						<input type="text" name="sd_page_title" value="{$query_data.sd_page_title}" size="50" style="width:75%;" />｜とことん車検ナビ
						<div class="submit_box"><input type="submit" name="submit01" value="設定" /></div>
						<input type="hidden" name="sid" value="{$sid}" />
					</form>
				</td>
			  </tr>
			  <tr>
                <td class="subdetail_label">ページキーワード</td>
                <td class="subdetail_value">
					<form action="/admin/detail/locate_keywords_change/" method="post">
					<table cellpadding="0" cellspacing="0" border="0" class="keyTab">
					<tr>
						<th>1(例)横浜市</th>
						<th>2(例)車検</th>
						<th>3(例)費用</th>
					...
					<tr>
						<td><input type="text" name="sd_page_keyword01" value="{$query_data.sd_page_keyword01}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword02" value="{$query_data.sd_page_keyword02}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword03" value="{$query_data.sd_page_keyword03}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword04" value="{$query_data.sd_page_keyword04}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword05" value="{$query_data.sd_page_keyword05}" size="8" /></td>
					</tr>
					<tr>
						<th>6(例)軽自動車</th>
						<th>7(例)見積</th>
					...
					<tr>
						<td><input type="text" name="sd_page_keyword06" value="{$query_data.sd_page_keyword06}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword07" value="{$query_data.sd_page_keyword07}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword08" value="{$query_data.sd_page_keyword08}" size="8" /></td>
						<td><input type="text" name="sd_page_keyword09" value="{$query_data.sd_page_keyword09}" size="8" /></td>
						<td class="tabLabel">とことん車検ナビ</td>
					</tr>
					</table>
					<div class="submit_box"><input type="submit" name="submit02" value="設定" /></div>
					<input type="hidden" name="sid" value="{$sid}" />
					</form>
				</td>
			  </tr>
			  <tr>
                <td class="subdetail_label" rowspan="2">ページ<br />ディスクリプション</td>
                <td class="subdetail_label"><span class="red">※店舗の特徴を100文字以内に設定してください</span><br />(例)横浜で車検をするならスーパーステーションNT茅ヶ崎店のスーパーFLEX車検へお任せ下さい。軽自動車車検が驚きの33,195円から！さらに毎月、車検を実施して頂いたお客様にもれなく選べるプレゼント進呈中</td>
			  </tr>
			  <tr>
                <td class="subdetail_value">
				<form action="/admin/detail/locate_description_change/" method="post">
					<textarea rows="5" cols="4" name="sd_page_description" style="width:90%;">{$query_data.sd_page_description}</textarea>
					<div class="submit_box"><input type="submit" name="submit03" value="設定" /></div>
					<input type="hidden" name="sid" value="{$sid}" />
				</form>
				</td>
			  </tr>
		...
		{if $small2img_data.img_path != ""}<img src="{$small2img_data.img_path}" alt="{$query_data.sd_small_img02}" width="{$small2img_data.width}" height="{$small2img_data.height}" class="pic" />{else}&nbsp;{/if}</td>
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="subdetail_right">
              <tr>
                <td width="118" class="subdetail_label">「見積リクエスト報告」受信先<br />「見積返信メール」差出人※</td>
